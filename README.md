...
- [Getting Started](#getting-started)
  - [Envirionment Variables](#dotenv) - environment variable declaration for docker-compose to use
  - [Create directories on host](#dir-host) - create the directories that will be used for managing the data on the host
...
- [Running](#site) - what to expect after you deploy
...
> Check for default values on `.env` file.

### [Create directories on host](#dir-host)
...
> **NOTE**: there is an assumption that both the `domain.name` and `www.domain.name` are valid DNS endpoints. If this is not the case, you will need to edit two files prior to running the `letencrypt-init.sh` script.
